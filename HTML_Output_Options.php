...
class HTML_Output_Options
{
   ...
   {
      $write_filename = "/var/www/tools/reports/".$relative_file_name.".txt";
      
      $this->output_filename = "http://tools.noblenet.org/reports/".$relative_file_name.".txt";
      
      
      if (!$relative_file_name) $links_new_window = true;
      else $links_new_window = false;
   ...
   function WriteSingleCopyBlock($curr_bib, $curr_copy, $use_bib_data, $links_new_window)
   {
      ...
      if($links_new_window) $new_window = " target=\"_blank\" ";
		...
		{
			$html_file_data .= "<span class=\"label\">Copy Location:</span><span class=\"copy_loc\">".$curr_copy->GetCopyLocation()."</span>\n";
			if (!$this->GetWordPress()) $html_file_data .= "<br />";
		}
   ...
   function WriteCopyTableBlock($curr_bib, $links_new_window)
   {
      ...
      if($links_new_window) $new_window = " target=\"_blank\" ";
	   ...
	   if ($this->GetCoverImage() && strlen($curr_bib->GetCatalogLink()) > 0)
		...
		//copy location
		if ($this->GetCopyLocation())  $html_file_data .= "<th>Copy Location</th>\n";
   ...
   function GetEmailText($include_link)
   {
       ...
       else if ($this->GetInlineLayout())$message .="Format = Inline\n";
       else if ($this->GetGridLayout())$message .="Format = Grid Width=".$this->GetGridWidth()."\n";
       else if ($this->GetSliderLayout())$message .="Format = Slider \n";
       
       if ($this->GetGroupCopies())
       {
          if ($this->show_first_copy) $message .="Group Copies First\n";
          else if ($this->show_all_copies) $message .="Group Copies All\n";
       }
       
       if ($this->GetImageSize()) $message .="Image Size ".ucwords($this->GetImageSize())."\n";
       if ($this->GetWordPress())$message .="Word Press\n";
   ...
   function GetPreviewText()
   {
       ...
       if ($this->GetGroupCopies())
       {
          if ($this->show_first_copy) $message .="Group Copies First<br/>";
          else if ($this->show_all_copies) $message .="Group Copies All<br/>";
       }
       ...
       if ( $this->GetCopyLocation() )  $message .=" -- Copy Location<br/>";
       if ( $this->GetCopyStatus() )    $message .=" -- Copy Status<br/>";
   ...
   function GetHTMLText()
   {
       ...
       if ($this->GetGroupCopies())
       {
          if ($this->show_first_copy) $html_out .="Group Copies First<br />";
          else if ($this->show_all_copies) $html_out .="Group Copies All<br />";
       }
       
       if ($this->GetImageSize()) $html_out .="Image Size: ".ucwords($this->GetImageSize())."<br />";
       if ($this->GetWordPress())$html_out .="Word Press<br />";
